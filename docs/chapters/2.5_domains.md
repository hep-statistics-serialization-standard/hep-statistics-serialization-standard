...
The component `axes` itself is an array of ranges. These can be continuous- or discrete-valued. A continuous axis has the components::
    -   `name`: custom string 
    -   `max`: upper bound of range 
    -   `min`: lower bound of range. 
  Discrete-valued (categorical) axes have the components: 
    - `name`: custom string
