...
An analytic convolution distribution describes a finite sum of factorized
basis functions convolved with a common resolution model. Let $x$ be the
convolution variate and let $z$ denote any other variates on which the
coefficients depend. Its unnormalized density is

$$
\begin{aligned}
q(x,z) &= \sum_k c_k(z)
          \int_{-\infty}^{\infty} B_k(u) R(x-u)\,\mathrm{d}u, \\
p(x,z) &= \frac{q(x,z)}{\ensuremath{\mathcal{M}}}.
\end{aligned}
$$

The normalization factor $\ensuremath{\mathcal{M}}$ is evaluated over the
current HS<sup>3</sup> domain. Continuous variates are integrated using their
domain measure, and categorical variates are summed using counting measure.
Categorical parameters are held at their current value. The inner convolution
always spans the full real line, independently of the domain of $x$.

The word *analytic* identifies the factorized contract and the opportunity for
specialized convolution kernels. A conforming implementation may instead use
a numerical convolution, provided that it evaluates the same density to its
documented numerical accuracy.

-   `name`: custom unique string
-   `type`: `analytic_convolution_dist`
-   `x`: name of the convolution variate $x$
-   `resolution_model`: name of a [resolution model](#sec:resolution-models)
    $R$. The resolution model [must]{.smallcaps} use the same `x`.
-   `terms`: non-empty array of term objects. Every term has exactly the
    following components:
    -   `coefficient`: number or name of a real-valued parameter or function
        $c_k$. It [must]{.smallcaps} be independent of `x`. An
        [`indexed_function`](#func:indexed-function) can provide
        category-dependent coefficients.
    -   `basis`: an exponential-decay basis object as defined below.

For every allowed point in the model parameter space, $q$ [must]{.smallcaps}
be finite and non-negative throughout its domain and [must not]{.smallcaps} be
identically zero. Individual coefficients need not be non-negative if the
complete sum satisfies this condition. This distribution is never extended.
Event yields are represented with the existing `rate_extended_dist` wrapper.

##### Exponential-decay basis

An exponential-decay basis has the following components:

-   `type`: `exp_decay`
-   `decay_constant`: positive number or name of a positive real-valued
    parameter or function $\tau$
-   `side`: one of `positive`, `negative`, or `both`
-   `modifier`: optional modifier object. If omitted, the multiplier is one.

Before applying a modifier, the three envelopes are
...
For `sin`, `cos`, `sinh`, and `cosh`, the modifier object has a `type` with
that value, a required `rate`, and an optional `scale`, which defaults to one.
Both `rate` and `scale` are numbers or names of real-valued parameters or
functions. For modifier type $f$, the complete basis is

$$
B(u)=E_{\mathrm{side}}(u;\tau)
f\!\left(\text{scale}\times\text{rate}\times u\right).
$$

A `polynomial` modifier object has `type` set to `polynomial` and an `order`
of `1` or `2`. It means

$$
B(u)=E_{\mathrm{positive}}(u;\tau)(u/\tau)^{\text{order}}.
$$

The polynomial modifier is initially defined only for a `positive` envelope.
Modifier parameters and the decay constant [must]{.smallcaps} be independent
of `x`.

The following object is a Gaussian-smeared, two-sided decay:

```json title="Example: Gaussian-smeared decay"
{
  "distributions": [
    {
    ...
    },
    {
      ...
      "type": "analytic_convolution_dist",
      "x": "decay_time",
      "resolution_model": "time_resolution",
      "terms": [
        {
          "coefficient": 1.0,
          "basis": {
...
A four-term positive-time basis can represent hyperbolic and oscillatory
components with a common lifetime:

```json title="Example: Four-term decay basis"
{
  ...
  "terms": [
    {
      ...
      "basis": {
        ...
        "decay_constant": "lifetime",
        "side": "positive",
        "modifier": {"type": "cosh", "rate": "delta_gamma", "scale": 0.5}
      }
    },
    {
      ...
      "basis": {
        ...
        "decay_constant": "lifetime",
        "side": "positive",
        "modifier": {"type": "sinh", "rate": "delta_gamma", "scale": 0.5}
      }
    },
    {
      ...
      "basis": {
        ...
        "decay_constant": "lifetime",
        "side": "positive",
        "modifier": {"type": "cos", "rate": "delta_mass"}
      }
    },
    {
      ...
      "basis": {
        ...
        "decay_constant": "lifetime",
        "side": "positive",
        "modifier": {"type": "sin", "rate": "delta_mass"}
      }
    }
  ]
}
...
The next example uses the `tag_sign` indexed function from the
[Functions section](#func:indexed-function). For either category, the sum is
non-negative before convolution:

```json title="Example: Category-dependent coefficient"
{
  "name": "tagged_oscillation",
  "type": "analytic_convolution_dist",
  "x": "decay_time",
  "resolution_model": "time_resolution",
  "terms": [
...
An analytic convolution distribution is invalid if `terms` is empty, if its
resolution model uses a different convolution variate, if a decay constant is
not positive, or if a modifier is not one of the forms above. A polynomial
modifier with an unsupported order or a side other than `positive` is also
invalid.
