...
A decay convolution distribution describes the convolution of a restricted
exponential decay profile with a restricted inline kernel. It has the
...
where `envelope` defines $E$, and each entry of `terms` defines one pair
$(c_i,B_i)$. The `envelope` object has a positive `scale` and `support` equal
to `positive`, `negative`, or `both`. Since every supported envelope is
exponential, it has no `type` component.
...
-   `coefficient`: a number or the name of a real-valued parameter or function
    $c_i$
-   `basis`: an inline basis object $B_i$

A `basis` with `type` equal to `constant` has no additional components and
represents $B_i(u)=1$. A `basis` with `type` equal to `sin`, `cos`, `sinh`, or
`cosh` has a required `rate` and represents
$B_i(u)=g(\text{rate}\,u)$ for the selected function $g$.

The additive sum is constructed by multiplying the `coefficient` and `basis`
from each term and then adding the resulting products:

$$
A(u)=c_1B_1(u)+c_2B_2(u)+\cdots+c_nB_n(u).
$$

The unconvolved profile is the product $S(u)=E(u)A(u)$. The `scale`, basis
rates, and coefficients [must]{.smallcaps} be independent of `x`. A derived
quantity, such as half of another rate, is represented by an ordinary
function and referenced from `rate`.
...
Kernel objects are embedded in `decay_convolution_dist`, inherit its `x`,
...
variate. A decay convolution is invalid if `terms` is empty, a term does not
contain exactly one `coefficient` and one supported `basis`, a parameter that
is required to be independent of `x` depends on `x`, an envelope scale or
Gaussian width is not positive, or a kernel mixture is empty or violates the
normalized non-negative coefficient convention. For either type, the
resulting density [must]{.smallcaps} be finite, non-negative, and nonzero.
