...
A convolution distribution combines a source distribution $f$ with a kernel
$K$ along one continuous variate $x$. Its unnormalized density and normalized
probability density are

$$
\begin{aligned}
q(x) &= \int_{-\infty}^{\infty} f(u)K(x-u)\,\mathrm{d}u, \\
...
fast Fourier transform. It is the preferred convolution representation unless
the source and kernel satisfy the portable analytic profile specified below.

-   `name`: custom unique string
-   `type`: `fft_convolution_dist`
-   `x`: name of the convolution variate $x$
-   `distribution`: name of the source distribution $f$
-   `kernel`: name of the kernel distribution $K$

Both referenced distributions [must]{.smallcaps} be non-extended and use `x`
as their convolution variate. Numerical choices such as the transform grid,
buffering, caching, and interpolation are implementation details and are not
part of this representation. Implementations [must]{.smallcaps} document the
numerical accuracy of their evaluation.
...
convolution with a restricted source and kernel representation for which
specialized implementations are available. It uses the same outer keys as
`fft_convolution_dist`:

-   `name`: custom unique string
-   `type`: `analytic_convolution_dist`
-   `x`: name of the convolution variate $x$
-   `distribution`: name of a non-extended `density_function_dist` whose
    `function` is a
    [`factorized_sum`](#func:factorized-sum){reference-type="ref"
    reference="func:factorized-sum"} satisfying the portable profile below
-   `kernel`: an inline kernel object satisfying the kernel catalog below

The `factorized_sum` [must]{.smallcaps} use the same `x`. Its portable
analytic profile requires:

-   an inline `factor` with `type` equal to `exponential`, a positive `scale`,
    and `support` equal to `positive`, `negative`, or `both`; and
-   `summands` whose entries are numbers or inline objects with `type` equal
    to `sin`, `cos`, `sinh`, or `cosh` and a required `rate`.
...
An inline summand of type $g$ represents $g(\text{rate}\,u)$. The `scale`,
`rate`, and coefficients are numbers or names of real-valued parameters or
functions and [must]{.smallcaps} be independent of `x`. A derived quantity,
such as half of another rate, is represented by an ordinary function and
referenced from `rate`.

For every allowed point in parameter space, the convolved density
$q$ [must]{.smallcaps} be finite and non-negative throughout its domain and
[must not]{.smallcaps} be identically zero. Individual coefficients may be
negative if the complete density satisfies these requirements. An
implementation may reject an analytic profile outside the catalog above
rather than providing a numerical fallback.
...
A Gaussian-smeared decay uses the same source distribution with an embedded
Gaussian kernel:
...
the existing `rate_extended_dist` wrapper. A convolution representation is
invalid if an operand is extended, its convolution variable does not match
`x`, or its resulting density is not finite, non-negative, and nonzero. An
analytic representation is also invalid if the factorized sum is empty, its
`summands` and `coefficients` have different lengths, a coefficient depends on
`x`, a scale or width is not positive, or its kernel mixture is empty, has an
invalid coefficient count, or has weights outside the normalized non-negative
convention.

##### Migration from earlier draft types

The following draft types are replaced rather than retained as aliases:

| Earlier type | Translation |
|---|---|
| `decay_dist` | Express the exponential source as a `factorized_sum`, wrap it in a `density_function_dist`, and reference it from an `analytic_convolution_dist`. Translate the former decay direction to `positive`, `both`, or `negative` support. |
| `gauss_model_function` | Embed a `gaussian` kernel with the same mean and width in the consuming `analytic_convolution_dist`. |
| `truth_model_function` | Embed a `delta` kernel in the consuming `analytic_convolution_dist`. |
| `mixture_model` | Embed its normalized resolution components in a `mixture` kernel. If it encoded an event yield, move that yield to a `rate_extended_dist` around the completed convolution. |
