...
All axis types [must]{.smallcaps} include:
...
-   Exactly one binning specification [must]{.smallcaps} be present:
...
-   Neither `nbins` nor `edges` [must not]{.smallcaps} be omitted
...
-   `axes`: array of axis structs. Each struct [must]{.smallcaps} contain `name`, `min`, `max` as defined in [Axis Specifications](#sec:axes). Binning fields (`nbins`, `edges`) [must not]{.smallcaps} be present.
...
-   `axes`: array of axis structs. Each struct [must]{.smallcaps} contain `name`, `min`, `max` and exactly one binning specification as defined in [Axis Specifications](#sec:axes):
