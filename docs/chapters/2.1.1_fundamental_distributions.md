...
This function is similar to the Poisson distribution (see [Poisson
distribution](#dist:poisson){reference-type="ref"
reference="dist:poisson"}), but can accept non-integer values for
$x$. Notably, the differences between the two might be significant for
small values of $x$ (below $1$). Nevertheless, the distribution is
...
$$
\begin{aligned} \text{GaussianPdf}(x, \mu, \sigma) = \frac{1}{\ensuremath{\mathcal{M}}} \exp\left(-\frac{(x-\mu)^2}{2\sigma^2}\right) \end{aligned}
$$
...
$$
\begin{aligned}
\text{GeneralizedNormalPdf}(x, \mu, \alpha, \beta) = \frac{1}{\ensuremath{\mathcal{M}}} \exp\left\{ -\left( \frac{|x-\mu|}{\alpha} \right)^\beta \right\}
\end{aligned}
$$

cf. the [Wikipedia definition](https://en.wikipedia.org/wiki/Generalized_normal_distribution).

This form is of particular use with values of $\beta \sim 8$ to generate a flat-topped distribution useful
for "2 point" parameter-constraints between models where there is no clear preference but their average
should not be particularly favoured. (In such usage, the template points should ideally be located at $|x| < \pm 1$,
to avoid disfavouring a pure solution.)
...
-   `beta`: power $\beta$ to which the exponent is raised, $\beta = 2$ corresponding to the $\text{GaussianPdf}$ shape.
...
#### Callable amplitude model

{% include-markdown "parts/amplitude.md" %}

#### HAMMER distribution

{% include-markdown "parts/hammer.md" %}
...
Eq. [(Breit-Wigner)](#eq:relativistic_breit_wigner_dist){reference-type="eqref" reference="eq:relativistic_breit_wigner_dist"} accounts for a jacobian of transformation from $m^2$ to $m$. The width term $\Gamma_1(m)$ adds for the phase space factor for the channel of interest 
...
Eq. [(Breit-Wigner)](#eq:relativistic_breit_wigner_dist){reference-type="eqref" reference="eq:relativistic_breit_wigner_dist"}, where the `[channels]` argument contains a single channel with $m_1=0$, $m_2=0$, and $l=0$. 
