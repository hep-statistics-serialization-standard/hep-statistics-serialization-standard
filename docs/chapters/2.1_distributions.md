...
An example for a two-dimensional multivariate distribution `mvnormal1` that has two free parameters `param_a` and `param_b` and might be used as a forward model - and to construct a likelihood when combined with data that has fields/columns/axes `obs_x` and `obs_y`:
...
"distributions":[
	...
    {
        ...
        "mean": ["param_a", "param_b"],
        "covariances": [[1.0, 0.3], [0.3, 2.0]],
    "x": ["obs_x", "obs_y"]
  },
	... 
] 
...
An example of a distribution `gauss1` that has no free parameters and might be used as a prior for a fit parameter `param_a`:
...
Names explicitly assigned to entries of variates of a distribution, like `obs_x`, `obs_y` and `param_a` in the examples above, introduce individual variate names in the global namespace of the document. They [must]{.smallcaps} be unique, also in respect to other object (distributions, functions, etc.) names (see Section [Unique names](#sec:global-namespace)).
